...
function querySearch(request,response) {
...
path.set("/querySearch",querySearch);
...
function queryByTag(request,response) {
    ...
    tagsDao.queryTag(params.tag,function (result) {
        ...
        if (result == null || result.length ==0) {
            ...
            console.log(result[0].int)
            tagBlogMappingDao.queryByTag(result[0].int,parseInt(params.page),parseInt(params.pageSize),function (result) {
                // console.log(result)
                var blogList = [];
                console.log(result)
