...
function deleteBlogById(request,response) {
    var params = url.parse(request.url,true).query;
    blogDao.deleteBlogById(parseInt(params.id),function (result) {
        response.writeHead(200);
        response.write(respUtil.writeResult("success","查询成功",result));
        response.end();
    })
...
function updateBlog(request,response) {
    var params = url.parse(request.url,true).query;
    blogDao.updateBlog(params.title,params.content,params.tags,params.ctime,params.id, function (result) {
        response.writeHead(200);
        response.write(respUtil.writeResult("success","查询成功",result));
        response.end();
    })
...
function personRecord(request,response) {
    var params = url.parse(request.url,true).query;
    blogDao.personRecord(params.userName, function (result) {
        response.writeHead(200);
        response.write(respUtil.writeResult("success","查询成功",result));
        response.end();
    })
}
path.set("/personRecord",personRecord);


function getpersonRecord(request,response) {
    var params = url.parse(request.url,true).query;
    blogDao.getpersonRecord(params.id,function (result) {
        response.writeHead(200);
        response.write(respUtil.writeResult("success","查询成功",result));
        response.end();
    })
}
path.set("/getpersonRecord",getpersonRecord);
